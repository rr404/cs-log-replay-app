...
const parserList = $('parser-list');
const scenarioList = $('scenario-list');
const statusBadge = $('status-badge');
const lineCountEl = $('line-count');
const statsBar = $('stats-bar');
const alertsContainer = $('alerts-container');
const bucketsContainer = $('buckets-container');
// ---------------------------------------------------------------------------
// File loading
// ---------------------------------------------------------------------------
function setupFileInput(inputId, dropZoneId, files, listEl, kind) {
    const input = $(inputId);
...
function renderFileList(files, listEl, kind) {
    listEl.innerHTML = '';
...
// Drag-to-reorder
// ---------------------------------------------------------------------------
function setupDragReorder(listEl, files, kind) {
    let dragSrcIndex = -1;
...
async function runReplay() {
    ...
    const logType = $('log-type-select').value;
    const keepState = $('keep-state').checked;
    setStatus('running');
...
function alertCard(a, idx) {
    ...
    const sources = a.sources.map(s => `<span class="tag tag-source">${escHtml(s.value || s.ip)}</span>`).join('');
    ...
    const tableRows = rows.map(([k, v]) => `<tr><td>${escHtml(k)}</td><td>${escHtml(v)}</td></tr>`).join('');
    return `
...
// ---------------------------------------------------------------------------
function renderBucketReport(report) {
    if (report.length === 0) {
