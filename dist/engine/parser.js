...
import { evalBool, evalString } from './expr';
 ...
 */
export function parseParserYAML(yamlText, defaultStage = 's01-parse') {
...
function buildLocalGrok(patternSyntax) {
...
function compileNode(cfg, inheritedStage, _grok) {
 ...
 */
export function runParsers(event, parsersByStage, stages) {
    if (stages.length === 0)
        return event;
    // Set initial stage if not set
    if (!event.Stage)
        event.Stage = stages[0];
    event.Process = false;
    ...
    for (const stage of stages) {
        ...
        if (eventStageIdx > currentStageIdx)
            continue;
        ...
        for (const node of nodes) {
            ...
            if (ret) {
                ...
                if (node.onsuccess === 'next_stage') {
                    console.info(`[parser]     onsuccess=next_stage → advancing stage`);
                    break;
                }
            }
            else {
...
function processNode(node, event) {
    ...
    // 6. Advance stage if next_stage
    if (node.onsuccess === 'next_stage') {
        // The stage advancement is handled in the outer loop by detecting event.Stage change
        // Actually: we need to set event.Stage here and the outer loop will detect it
        // (mirrors the Go: p.Stage = ctx.Stages[idx+1])
        // We'll signal via a sentinel — actually we just leave it; the outer loop
        // checks event.Stage !== stage to detect the break.
        // For next_stage within a node, the node needs to advance the stage on the event.
        // We need the stages list here. We'll handle this via a closure trick.
        // For simplicity, mark event with a special field indicating advancement requested.
        event._nextStageRequested = true;
    }
    return true;
}
/** Evaluate grok pattern against the event. Returns [nodeState, grokOK]. */
function processGrok(grok, event, nodeLabel = '') {
    let input;
...
function processLeaves(parent, event, parentState, parentHasOKGrok) {
    let leafState = parentState;
    for (const child of parent.leaves) {
        const ret = processNode(child, event);
        if (ret) {
            leafState = true;
            if (parent.onsuccess === 'next_stage')
                break;
        }
...
function checkWhitelist(wl, event) {
 ...
 */
export function applyStatic(cfg, event, nodeLabel = '') {
...
function callEnrichMethod(method, input, event) {
    if (method === 'ParseDate') {
    ...
    if (method === 'UnmarshalJSON') {
        try {
            event.Unmarshaled = JSON.parse(input);
        }
        catch { /* ignore */ }
        return;
    }
...
function setEventField(event, path, value) {
    // Supported paths: StrTime, Process, MarshaledTime, Meta.key, Parsed.key, Enriched.key
    if (path === 'StrTime') {
        event.StrTime = value;
        return;
    }
    if (path === 'MarshaledTime') {
        event.MarshaledTime = value;
        return;
    }
    if (path === 'Process') {
        event.Process = value === 'true';
        return;
    }
 ...
 */
export function organiseByStage(parsers) {
