...
export function loadBucketFactories(yamlText, overflowChan) {
...
function newBucket(factory, mapkey) {
 ...
 */
function timeMachineAllow(bucket, eventTime) {
 ...
 */
export function pourItemToHolders(event, factories, store) {
    let poured = false;
    ...
    for (const factory of factories) {
        ...
        console.info(`[bucket]   scenario "${spec.name}" groupby="${groupby}" → poured  count=${bucket.totalCount}  tokens=${bucket.tokens.toFixed(2)}/${bucket.capacity}  overflow=${overflow}`);
        if (overflow) {
...
function pourIntoBucket(bucket, event, eventTime) {
    const spec = bucket.factory.spec;
...
function handleOverflow(bucket, factory, store) {
    ...
    console.info(`[bucket] *** OVERFLOW "${name}"  groupby="${bucket.mapkey.slice(0, 8)}"  events=${bucket.queue.length}  at=${bucket.ovflwTs?.toISOString() ?? '?'} ***`);
...
function resetBucket(bucket, store) {
 ...
 */
export function garbageCollectBuckets(currentEventTime, store) {
    for (const bucket of store.snapshot()) {
        ...
        const capacity = spec.capacity ?? 0;
        if (capacity <= 0)
            continue; // counter/conditional: no GC by token count
        const leakspeedMs = bucket.factory.leakspeedMs;
        if (leakspeedMs <= 0 || !bucket.lastRefillTime)
            continue;
...
function makeBucketKey(filter, groupby, name) {
