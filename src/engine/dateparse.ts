...
export interface ParsedDate {
  iso: string   // ISO8601 string (what MarshaledTime is set to)
  date: Date
}
...
export function genDateParse(s: string): ParsedDate | null {
  s = s.trim()
  if (!s) return null
  ...
  if (apacheDate) return apacheDate
...
function toISO(d: Date): ParsedDate | null {
...
function tryApache(s: string): ParsedDate | null {
...
function tryUnixAsctime(s: string): ParsedDate | null {
...
function trySyslog(s: string): ParsedDate | null {
...
function tryYMD(s: string): ParsedDate | null {
...
function tryUnixTimestamp(s: string): ParsedDate | null {
