 ...
 */
export function evalExpr(expression: string, env: ExprEnv): unknown {
  try {
...
export function evalBool(expression: string, env: ExprEnv): boolean {
  const result = evalExpr(expression, env)
  return !!result
}

/** Evaluate to a string. Returns '' on error/non-string. */
export function evalString(expression: string, env: ExprEnv): string {
  const result = evalExpr(expression, env)
...
function tokenize(src: string): Token[] {
  ...
  while (i < src.length) {
    ...
    if (src[i] === '!' && src[i+1] === '=') { tokens.push({ kind: 'NEQ', value: '!=' }); i += 2; continue }
    if (src[i] === '=' && src[i+1] === '=') { tokens.push({ kind: 'EQ',  value: '==' }); i += 2; continue }
    if (src[i] === '<' && src[i+1] === '=') { tokens.push({ kind: 'LTE', value: '<=' }); i += 2; continue }
    if (src[i] === '>' && src[i+1] === '=') { tokens.push({ kind: 'GTE', value: '>=' }); i += 2; continue }
    if (src[i] === '&' && src[i+1] === '&') { tokens.push({ kind: 'AND', value: '&&' }); i += 2; continue }
    if (src[i] === '|' && src[i+1] === '|') { tokens.push({ kind: 'OR',  value: '||' }); i += 2; continue }
    ...
    if (ch === '[') { tokens.push({ kind: 'LBRACKET', value: '['  }); i++; continue }
    if (ch === ']') { tokens.push({ kind: 'RBRACKET', value: ']'  }); i++; continue }
    ...
    if (ch === '?') { tokens.push({ kind: 'QUESTION', value: '?'  }); i++; continue }
    if (ch === ':') { tokens.push({ kind: 'COLON',    value: ':'  }); i++; continue }
    if (ch === '+') { tokens.push({ kind: 'PLUS',     value: '+'  }); i++; continue }
    if (ch === '-') { tokens.push({ kind: 'MINUS',    value: '-'  }); i++; continue }
    if (ch === '*') { tokens.push({ kind: 'STAR',     value: '*'  }); i++; continue }
    if (ch === '/') { tokens.push({ kind: 'SLASH',    value: '/'  }); i++; continue }
    if (ch === '%') { tokens.push({ kind: 'PERCENT',  value: '%'  }); i++; continue }
    ...
    if (/[a-zA-Z_]/.test(ch)) {
      ...
      if (word === 'nil' || word === 'null') { tokens.push({ kind: 'NULL', value: word }); i = j; continue }
      if (word === 'in')    {
        // look ahead: "not in" is NOTIN
        tokens.push({ kind: 'IN', value: 'in' }); i = j; continue
      }
      if (word === 'not') {
        // could be "not in"
        let k = j
        ...
        if (src.slice(k, k + 2) === 'in') {
          tokens.push({ kind: 'NOTIN', value: 'not in' }); i = k + 2; continue
        }
        tokens.push({ kind: 'NOT', value: 'not' }); i = j; continue
      }
...
type ASTNode =
  ...
  | { t: 'null' }
  | { t: 'ident';  v: string }
  | { t: 'field';  obj: ASTNode; key: ASTNode }   // obj.key or obj["key"]
  | { t: 'call';   fn: string;  args: ASTNode[] }
  | { t: 'method'; obj: ASTNode; method: string; args: ASTNode[] }
  | { t: 'unary';  op: string;  expr: ASTNode }
  | { t: 'binary'; op: string;  left: ASTNode; right: ASTNode }
  | { t: 'ternary'; cond: ASTNode; then: ASTNode; else_: ASTNode }
  | { t: 'array';  items: ASTNode[] }
...
class ExprParser {
  ...
  private parseComparison(): ASTNode {
    ...
    const ops: TokenKind[] = ['EQ','NEQ','LT','LTE','GT','GTE']
  ...
  private parsePrimary(): ASTNode {
    const t = this.peek()
    if (t.kind === 'NUM')  { this.consume(); return { t: 'num', v: parseFloat(t.value) } }
    if (t.kind === 'STR')  { this.consume(); return { t: 'str', v: t.value } }
    if (t.kind === 'BOOL') { this.consume(); return { t: 'bool', v: t.value === 'true' } }
    if (t.kind === 'NULL') { this.consume(); return { t: 'null' } }
    if (t.kind === 'LPAREN') {
      this.consume()
...
function evalNode(node: ASTNode, env: ExprEnv): unknown {
  switch (node.t) {
    ...
    case 'unary': {
      if (node.op === '!') return !evalNode(node.expr, env)
      if (node.op === '-') return -(evalNode(node.expr, env) as number)
      return undefined
    }
    ...
    case 'binary': {
      ...
      if (op === '&&') return !!evalNode(node.left, env) && !!evalNode(node.right, env)
      if (op === '||') return !!evalNode(node.left, env) || !!evalNode(node.right, env)
      ...
      if (op === 'in') {
        ...
        if (typeof right === 'object' && right !== null) return left as string in (right as object)
      ...
      if (op === '-')  return (left as number) - (right as number)
      if (op === '*')  return (left as number) * (right as number)
      if (op === '/')  return (right as number) !== 0 ? (left as number) / (right as number) : 0
      if (op === '%')  return (left as number) % (right as number)
      return undefined
    }
  }
...
function evalMethod(method: string, obj: unknown, args: unknown[]): unknown {
  if (typeof obj === 'string') {
    if (method === 'startsWith') return obj.startsWith(args[0] as string)
    if (method === 'endsWith')   return obj.endsWith(args[0] as string)
    if (method === 'contains')   return obj.includes(args[0] as string)
    if (method === 'matches')    {
      try { return new RegExp(args[0] as string).test(obj) } catch { return false }
    }
    if (method === 'hasPrefix')  return obj.startsWith(args[0] as string)
    if (method === 'hasSuffix')  return obj.endsWith(args[0] as string)
    if (method === 'toLower')    return obj.toLowerCase()
    if (method === 'toUpper')    return obj.toUpperCase()
    if (method === 'trimSpace')  return obj.trim()
  ...
  if (typeof obj === 'object' && obj !== null) {
    if (method === 'contains') return args[0] as string in (obj as object)
  }
  return undefined
}

function evalBuiltinCall(fn: string, args: unknown[], env: ExprEnv = {}): unknown {
  switch (fn) {
    case 'len':   {
      const v = args[0]
      if (typeof v === 'string') return v.length
      if (Array.isArray(v)) return v.length
      if (typeof v === 'object' && v !== null) return Object.keys(v as object).length
      return 0
    }
    ...
    case 'KeyExists': {
      ...
      const map = args[1] as Record<string, unknown>
      if (!map || typeof map !== 'object') return false
      return key in map
    }
    case 'IpInRange': {
      const ip = args[0] as string
      const cidr = args[1] as string
    ...
    case 'UnmarshalJSON': {
      ...
      const targetMap = args[1] as Record<string, unknown> | null | undefined
      const key = String(args[2] ?? '')
      try {
        const parsed = JSON.parse(jsonStr)
        // Write into the event's Unmarshaled map if we can reach it
        const evt = env['evt'] as Record<string, unknown> | undefined
        if (evt && typeof evt['Unmarshaled'] === 'object' && evt['Unmarshaled'] !== null) {
          (evt['Unmarshaled'] as Record<string, unknown>)[key] = parsed
